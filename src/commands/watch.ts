...
import { assertSetup, pathExists, Logger, ASSET_DIRS, loadConfig, spawnBuilder, spawnTranspiler, spawnTranspilingWatcher } from "../common";
...
export const runWatch = async () => {
    ...
    try {
        await spawnTranspilingWatcher(`./src/`, `./dev/${config.project.slug}_dev/Scripts/`, config.project.transpile || config.project.template === "typescript");
    } catch (e) {
        Logger.error("something went wrong with the watcher");
        throw e;
    }
};
